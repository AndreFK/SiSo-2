...
BOOL CALLBACK DialogProcLink(HWND hDlg, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			...
			{
			case 2:
				val = CreateSymbolicLink(fullPathOutput, fullPathOrigin, 0);//0 for symlink to files
				//erro = GetLastErrorAsString();
				//val = CreateSymbolicLinkW(fullPathOrigin, fullPathOutput, 0);
				break;
			case 3:
			...
			case 4:
			{
				...
				EndDialog(hDlg, 0);
				if (!si)
					MessageBox(0, L"Link Duro", L"No se pudo crear este link", 0);
				return FALSE;
			}
			default:
				break;
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDM_ATRAS:
			...
			loadChildForListView(nPath, drive);
			break;
			...
			if (msgboxID == IDYES)
			{
				...
				while (iPos != -1) {
					...
					if (findNameItem(g_nameFile, StartPoint)) {
						...
						int res = silently_remove_directory(path);
